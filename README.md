...
| :--- | :--- | :--- | :--- | :--- |
| Shell | 12820 | 60.92% | 1916 | 77.79% |
| Nix | 3804 | 18.08% | 167 | 6.78% |
| Python | 1239 | 5.89% | 329 | 13.36% |
| CSS | 1015 | 4.82% | 1 | 0.04% |
| Markdown | 540 | 2.57% | 0 | 0.00% |
| JSONC | 436 | 2.07% | 0 | 0.00% |
| JSON | 402 | 1.91% | 0 | 0.00% |
| Patch | 396 | 1.88% | 0 | 0.00% |
| AWK | 233 | 1.11% | 46 | 1.87% |
| XML | 78 | 0.37% | 0 | 0.00% |
| JavaScript | 42 | 0.20% | 4 | 0.16% |
| Julia | 25 | 0.12% | 0 | 0.00% |
| TOML | 13 | 0.06% | 0 | 0.00% |
| **Total** | **21043** | **100.00%** | **2463** | **100.00%** |
<!-- STATS END -->
...
* `lfs_autoremove` (called `autoremove` in VM) is used to remove one or more packages from the LFS system. Syntax is `lfs_autoremove [--dry-run] [-f] <package_name> [<package_name> ...]`. It automatically checks for libraries provided by other packages that depend on the package being removed; if such library(s) exist, the removal is cancelled. `-f` removes this library-checking behaviour. `--dry-run` lists what actions would be taken if the command was issued without this argument. Multiple package names may be supplied and each will be removed in turn.
* `du_pkg` reports the total disk space used by all files belonging to a given package, using `/var/lib/custom-packages/` (or `/var/lib/book-packages/`) to determine ownership. Syntax is `du_pkg <package_name>`.
